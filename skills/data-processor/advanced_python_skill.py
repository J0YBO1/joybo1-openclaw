#!/usr/bin/env python3
"""
高级Python技能示例 - data-processor
一个功能丰富的数据处理工具，展示Python高级特性在OpenClaw中的应用
"""
...
class DataProcessor:
    ...
    @validate_input
    @timing_decorator
    def sort_by(self, key: str, reverse: bool = False) -> List[Dict[str, Any]]:
        """按键排序数据"""
...
示例:
  # 过滤数据
  python advanced_python_skill.py data.json --filter '{"status": "active"}'
  
  # 分组并聚合
  python advanced_python_skill.py data.json --group-by category --aggregate price --operation avg
  
  # 统计摘要
  python advanced_python_skill.py data.json --stats price
  
  # 排序并输出
  python advanced_python_skill.py data.json --sort-by price --reverse --output results.json
        """
    )
    ...
    # 操作选项
    parser.add_argument("--filter", type=json.loads, help="过滤条件 (JSON格式)")
...
def main():
    ...
    try:
        ...
        # 执行操作
        if args.filter:
            filter_result = processor.filter_data(args.filter, verbose=args.verbose)
            result = filter_result["result"]
            operation = f"过滤: {args.filter}"
            if args.verbose:
                print(f"过滤后剩余 {len(result)} 条记录")
